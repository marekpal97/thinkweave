...
*Git tie-in:* a decision carries `commit_refs:` only when `mem_extract` ran over a session whose hooks captured commits (`outcome: committed`). Decisions created without a git context — via `/capture`, direct `mem_create`, non-code conversations, or retroactive authorship inside an existing session note — stay `proposed` with no `commit_refs:` and no judge verdict, **by design**. `outcome: abandoned` decisions also stay `proposed` (no code change expected); promotion to `deprecated` is manual.
