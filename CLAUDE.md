...
*Predicted-outcome (RLVR substrate, MVP shipped 2026-05-14):* decisions can optionally carry a `predicted_outcome:` string — a forward-looking claim about what should be true if the decision is right (e.g. "tests will pass", "this will land in one commit"). The judge maps it to `prediction_match ∈ {confirmed, partial, contradicted, unevaluable}` using two narrow keyword families (test, commit) crossed with structural evidence (`tested`, `verdict`). Conservative by design — `unevaluable` is a legit common terminal value; only narrow deterministic rules promote off it (no LLM, no semantic similarity). Feeds `mem rlvr export`. Composed inline by `/mem-wrap` from session conversation; never prompted for.
