...
**Theme.** `type: theme`, prefix `thm-`, statuses `active → dormant → resolved` / `merged-into:thm-X` — **status changes by hand only** (the 2026-05-30 teardown dropped all automatic lifecycle; mint + extend are the only automatic mutations, and merging a duplicate is the one status change `/themes-resolve` makes). Canonical themes live at `vault/themes/{thm-XXXX}-{slug}.md` regardless of project. Three sections: `## Essence`, `## Catalyst log` (same grammar as concept-hub log), `## Open questions`. Decisions implementing a theme carry `implements: [thm-XXXX]`. `/themes-resolve` is the periodic hygiene pass — merge near-duplicate themes and rewrite stale essences (no candidate promotion, no dormancy detection).
...
| `/dream` | vault_synthesis | — | — | Two-phase subagent orchestrator. **Phase 1** (synthesis) fans out 5 workers in parallel — `dream-{promotion,merge,theme,essence,priority}-worker` — merges plan fragments, runs `mem dream apply` (one index rebuild, one maintenance.jsonl line). **Phase 2** (composition + consumption) fans out 3 workers in dependency waves — `dream-wrap-worker` (catch-up unwrapped sessions) + `dream-judge-worker` (drain rejudge queue) in parallel, then `dream-digest-worker` (compose `type: digest` note at `vault/projects/<p>/digests/YYYY-MM-DD.md`). One cron entry replaces three (`/dream`, `/mem-wrap` catch-up, `/judge-prediction --drain`). Headless-safe; never prompts. |
| `/mem-resolve-concepts` | ontology_hygiene | — | — | Concept and ontology hygiene (manual / interactive). Periodic promotion + drift merges run inside `/dream` phase 1 via `dream-promotion-worker` + `dream-merge-worker`. |
| `/themes-resolve` | theme_synthesis | — | — | Theme dedup + essence-refresh hygiene (manual). Note: routine essence rewrites run inside `/dream` phase 1 via `dream-essence-worker`; this skill is for the harder dedup work. |
...
The CLI exposes **45 subcommands** total via `_DISPATCH` in `surfaces/cli/__init__.py`. Agents work primarily through MCP tools (see below); the CLI is for setup, admin, and the small set of operations without MCP parity.
