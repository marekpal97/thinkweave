...
*Source-coupled theme floating:* whether a source type auto-floats theme candidates is controlled by the `temporal_grain` field on `SourceTypeSpec`. Event-shaped types (`substack`, `news`, `newsletter-events`) get `temporal_grain='event'` — their post-ingest path runs a deterministic cluster check (≥3 recent sources sharing ≥2 concepts, no covering theme) and writes a candidate stub. Concept-shaped types (`paper`, `repo`, `article`, `newsletter-concepts`) get `temporal_grain='concept'` — concept hubs handle them, no theme floating. Conversation-style intake gets `temporal_grain='none'` (no auto-floating). The disambiguation test below is preserved unchanged: candidate stubs propose themes, they don't redefine what a theme is.
...
*Predicted-outcome (RLVR substrate, MVP shipped 2026-05-14; hardened 2026-05-21):* decisions can optionally carry a `predicted_outcome:` — a forward-looking claim about what should be true if the decision is right. Canonical shape is a structured dict: `{family: "test"|"commit", text: "...", polarity: "positive"|"negative"|null}`. The judge dispatches on `family` directly (no keyword sniff), reads `polarity` to invert the verdict for "tests will fail" / "won't land" style predictions, and maps the pair to `prediction_match ∈ {confirmed, partial, contradicted, unevaluable}` against structural evidence (`tested`, `verdict`, per-run test counts). `partial` fires when mixed-result test runs partially confirm a positive-polarity prediction (some passed, some failed). Bare-string `predicted_outcome:` still roundtrips for one release — the judge sniffs family with word-boundary regexes (so `pass the buffer` no longer misfires the test family) and downgrades to `unevaluable` whenever polarity is ambiguous. Conservative by design — `unevaluable` is a legit common terminal value; only narrow deterministic rules promote off it (no LLM, no semantic similarity). Feeds `mem rlvr export`. Composed inline by `/mem-wrap` from session conversation; never prompted for.

**Source.** External content: `paper`, `repo`, `article`, `conversation`, `substack`, `news`, `newsletter-events`, `newsletter-concepts`, … Routed by `src/personal_mem/sources/registry.py` (`SourceTypeSpec`). Three layouts: `flat`, `folder`, `author_folder`. Per-source-type behaviour (queue path, drain strategy, dedup keys) is overridable in `vault/.mem/sources.yaml`.

*Mail-connector intake (newsletters).* Email newsletters land via a provider-agnostic mail connector — `gmail` today (interactive OAuth via the `mcp__claude_ai_Gmail__*` MCP tools), `outlook` and `imap` are slots for later, no code differences beyond a small fetch-block branch in `/newsletter`. **The canonical filter is a per-type `senders:` allowlist** in `vault/.mem/sources.yaml` — each entry is an address or bare domain — and the skill composes `from:(s1 OR s2 OR ...)` for the connector's query. Empty allowlist + empty `mail_query` is a deliberate halt, not a whole-inbox fan-out. The optional `mail_query:` field is for extra filters (`is:unread`, etc.) ANDed on top. Three re-read guards stack: (1) **mail label** — `/newsletter` applies `processed_label` (default `mem-processed`) on the server after a successful drain; the next query excludes it; (2) **queue dedup** — `mem_queue(action="enqueue")` rejects on `dedup_keys: [message_id, url]`; (3) **`mem_search` backstop** — the worker short-circuits to `idempotent_skip` if `message_id` already exists in the vault. `lookback_days` (per source-type config, translated to provider syntax) bounds backfill — Gmail gets `newer_than:Nd`, IMAP gets `SINCE`. The Gmail connector requires interactive OAuth so `/newsletter` is interactive-only; headless cron use needs the `imap` connector.
...
| `/newsletter` | newsletter_inbox | newsletter-events, newsletter-concepts | acquire | Drain email-newsletter labels (mail connector → enqueue → fan-out workers → mark processed). Interactive (Gmail OAuth). |
