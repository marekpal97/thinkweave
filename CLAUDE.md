...
- **Hybrid** — `mem_search(query, mode='hybrid')`. Unsure → RRF fusion (k from `retrieval.rrf_k`, default 60).
...
| `/dream` | vault_synthesis | — | — | Two-phase subagent orchestrator. **Phase 1** (synthesis) fans out 5 workers in parallel — `dream-{promotion,merge,theme,essence,priority}-worker` — merges plan fragments, runs `mem dream apply` (one index rebuild, one maintenance.jsonl line). **Phase 2** (composition + consumption) fans out 5 workers in dependency waves — `dream-wrap-worker` (catch-up unwrapped sessions) + `dream-judge-worker` (drain rejudge queue) + `dream-seam-link-worker` (stitch cross-parent linkage on freshly-folded hubs via `mem hubs apply-linkage`) + `dream-seam-worker` (reconcile the CC-auto-memory↔vault **memory seam** — judge each dirty CC fact's vault twin, write the durable map via `mem seam commit`) in parallel, then `dream-digest-worker` (compose vault-global `type: digest` notes at `vault/digests/YYYY-MM-DD-<grain>.md`, one per non-empty grain). One cron entry replaces three (`/dream`, `/mem-wrap` catch-up, `/judge-prediction --drain`). Headless-safe; never prompts. |
...
| `/enrich-notes` | concept_enrichment_inline | — | — | Inline LLM concept enrichment via the running model. The `mem enrich --via inline` route; pairs with `--via batch` (wrapper async fan-out). Picked automatically by `choose_route()` when no key OR ≤200 candidates. |
| `/import-chatgpt` | chatgpt_import_inline | — | import | Inline ChatGPT-export import. The `mem import chatgpt --via inline` route; same role as `/enrich-notes` for ChatGPT data exports. |
| `/hubs-link` | hubs_linkage_inline | — | — | Inline temporal-DAG linkage for concept hubs. The `mem hubs link --via inline` route; pairs with `--via batch`. |

**Dual-route convention:** four CLI subcommands take `--via {inline,batch}`: `mem import claude-code --enrich`, `mem import chatgpt`, `mem enrich`, `mem hubs link`. `inline` = the corresponding CC skill above runs the work via the running model (no provider key required); `batch` = the wrapper (`core/agent_client.batch_completions_sync`) fans out N async completions to the configured provider (provider+model from `vault/config/api.yaml::overrides.<op>`). When `--via` is omitted, `operations/_backfill_route.choose_route` picks: explicit flag > size threshold + key presence > inline.
...
- **LLM provider abstraction.** Backfill and triage paths go through `core/agent_client.py` (AsyncOpenAI + per-provider `base_url`) configured via `vault/config/api.yaml`. Embeddings live in their own layer (`core/embedding_provider.py`, `EmbeddingProvider` protocol). **Carve-out:** `sources/extractors/gemini_extract.py` keeps direct `google.genai` — the Files API audio modality for podcast extraction has no chat-completion equivalent. The dormant YouTube Gemini fallback (`_call_gemini_for_youtube`) stays in the module as a documented opt-in fallback; the live YouTube path uses `youtube-transcript-api` (no LLM).
...
`mem init`, `mem hooks`, `mem schedule` directly — they belong in cron flows
or interactive admin. There is no MCP parity for these subcommands.
