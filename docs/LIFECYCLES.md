...
SessionStart is serve-once (#175, reworked per the PR #203 review). Harnesses emit SessionStart several times per logical conversation, but the initial context goes out exactly once per session: “I don't really see a need to ever serve up the startup context more than once per session … just have the initial context be served up once and from there if we need more thinkweave MCP/CLI can be used anytime.” `_handle_session_start` branches on the hook input's `source`: `startup`/`clear` (or absent — legacy harnesses) serve the full snapshot; `resume`/`compact` inject NOTHING — resume replays the transcript (the context is still in the window), and after compaction the agent retrieves what it needs on demand through the `weave_*` MCP/CLI retrieval surface. Each skipped lifecycle delivery buffers a zero-id `skipped_lifecycle` telemetry event (projecting no `context_served` rows), and the skip path builds no payload at all. Startup events record the `lifecycle` source and a `disposition` (`served_full` / `skipped_replay` / `skipped_lifecycle`). Replay guard — the once-per-session guarantee for `startup`/`clear`: a delivery's identity is `(session_id, lifecycle, served-id-set hash)` — not payload bytes, whose header timestamp would split identity across a minute boundary — stamped as `delivery_id` on the served event; an identical redelivery (double registration, post-archival replay) injects nothing and records one `skipped_replay` event per replay identity with zero ids, so exposure is never double-counted. Receipts come from the session's OWN streams only (the live buffer, else its archived folder among the newest few by the trailing `-YYYY-MM-DD` of the folder name — the leading component is a random uuid, so a bare name sort orders by coin flip); the receipt survives live-buffer archival because it rides inside the event line into `retrieval_log.jsonl`. The RLVR export unions `context_served` across the logical chain (`_chain_session_ids`, with the same real-wrap sibling exclusion the wrap path applies) so a decision minted in a later segment still sees the first segment's startup exposure, and `startup_token_est` skips replay telemetry. Prompt-time pushed context (`prompttime`) and agent-requested retrieval (`onthefly`) are different surfaces and are untouched by all of this.
