...
**Compaction-segment chain (#180).** A long claude.ai/code session spans several harness session UUIDs — each context compaction starts a new segment, buffering prompts under its own id. The chain key is `logical_session` (the transcript's `bridgeSessionId`, `cse_…`), stamped on every segment's session note at creation by the hooks; interactive CLI / Codex sessions have none. `wrap-finalize --verdicts` resolves the whole chain (id match ∪ shared `logical_session` ∪ a prior wrap's `segments:` list — minus siblings a REAL wrap already processed, `processed` with `auto_extracted` cleared: `bridgeSessionId` survives resumption across days, and an already-wrapped sibling's prompts were labeled by its own richer wrap), matches verdicts against prompts from every segment, appends each event to the file that holds its prompt, and writes the durable `segments: [uuid, …]` record onto the wrapped session note (union with what is stored — the record outlives any one join's span) — the same parent key the task-trace epic (#184) reads.
...
Email newsletters land via a provider-agnostic mail connector — `gmail` today, `outlook` and `imap` are slots. The `mail_poll` discover strategy *composes* the per-type Gmail query (sender allowlist → `from:(s1 OR s2 OR ...)`, optional `mail_query` extras, `-label:<processed_label>` exclusion, `newer_than:Nd` lookback) and emits a `mail_fetch_needed` plan. The `/newsletter` skill *executes* the plan against Gmail MCP — fetch threads, enqueue, drain, then `label_thread` on every queue item archived `done`, which includes items whose worker outcome was `idempotent_skip` (an already-briefed thread is handled; leaving it unlabelled would re-fetch it every run, since queue dedup only looks back 7 days). Empty allowlist + empty `mail_query` is a deliberate halt in the strategy, not a whole-inbox fan-out. Three re-read guards stack: (1) **mail label** (primary — survives queue wipes); (2) **queue dedup** on `[message_id, url]`; (3) **`weave_search(message_id)` backstop** in the worker. The Gmail connector's first-ever grant is interactive OAuth; the grant is then cached account-side and survives headless `claude -p` firings, so the rail is cron-safe — `/newsletter` ships as a (default-disabled) `newsletter` job in the scheduling registry, and its step-1 connector probe fails on one diagnostic line rather than prompting. The `imap` connector remains a slot for accounts with no Gmail, not a prerequisite for scheduling.

## Context-served (RLVR substrate)

Each session captures the notes served to it: a `type: startup` event per SessionStart delivery (notes in the SessionStart payload + `token_est`) plus one `type: retrieval` event per MCP retrieval call during the session (`weave_search`, `weave_context`, `weave_graph`, `weave_read`, `weave_timeline`, `weave_project_snapshot`). Buffered to the same per-session JSONL as action events; `archive_buffer` (Stop time / `weave_extract`) splits them into sibling `events.jsonl` and `retrieval_log.jsonl`. The Indexer projects every session's `retrieval_log.jsonl` into `context_served(session_id, note_id, source ∈ {startup, onthefly, prompttime, loop-prime, codex-startup}, ts)` — rebuildable from markdown. `/learn`'s reads land here as ordinary `onthefly` rows; `/brief` deliberately adds nothing beyond its own calls (a survey is not demand — dec-696bacfb). The RLVR row's `context.cited_onthefly_ids` / `cited_startup_only_ids` come from intersecting decision body wikilinks against this table; a note served both via startup and on-the-fly counts as onthefly (the stronger signal).

SessionStart is lifecycle-aware (#175). Harnesses emit SessionStart several times per logical conversation, so `_handle_session_start` branches on the hook input's `source`: `startup`/`clear` (or absent) serve the full snapshot; `resume`/`compact` serve a **delta** — only notes the logical-session chain (#180) hasn't been served yet, one line each, plus a single "N notes already in context" line. Resume and compact differ: resume replays the transcript (suppressed notes are genuinely back in the window, so they don't reappear even as text), while compaction evicted them — the compact delta additionally lists the suppressed ids as plain-text `weave_read`-to-restore references (never counted as exposure) and re-serves the tools manifest + retrieval footer. The already-served set is read from the chain's raw retrieval streams (each member folder's `retrieval_log.jsonl` + live buffers) — the same files `context_served` projects from, current the moment an event is buffered; the scan is bounded to the newest folders by the trailing `-YYYY-MM-DD` of the folder name (`_CHAIN_SCAN_RECENT` — the leading component is a random uuid, so a bare name sort orders by coin flip), and `startup`/`clear` never resolve the chain at all (only this session's own streams, for replay receipts) — SessionStart latency is syscall-shaped, and the call topology is pinned by tests. Each startup event records the `lifecycle` source and a `disposition` (`served_full` / `served_delta` / `skipped_replay`); delta events also stamp `chain_root` (the member with the earliest startup ts — not folder-name order, which can flip on same-day segments), and the projection keys those rows to the root — a resume/compact delivery's exposure belongs to the logical conversation, not the segment that buffered it. Replay guard: a delivery's identity is `(session_id, lifecycle, served-id-set hash)` — not payload bytes, whose header timestamp would split identity across a minute boundary — and resume/compact identities additionally fold in the live buffer's non-startup event count (the activity watermark): successive compactions of an unchanged vault serve identical deltas, and the conversation activity between them is what tells a genuinely new lifecycle event from a byte-duplicate double registration. Stamped as `delivery_id` on the served event; an identical redelivery (double registration, post-archival replay) injects nothing and records one `skipped_replay` event per replay identity with zero ids, so exposure is never double-counted. The receipt survives live-buffer archival because it rides inside the event line into `retrieval_log.jsonl`. The RLVR export unions `context_served` across the chain (`_chain_session_ids`, with the same real-wrap sibling exclusion the serve/wrap paths apply) so a decision minted in a later segment still sees root-homed exposure, and `startup_token_est` skips replay telemetry. Prompt-time pushed context (`prompttime`) and agent-requested retrieval (`onthefly`) are different surfaces and are untouched by all of this.
