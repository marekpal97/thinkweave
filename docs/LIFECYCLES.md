...
SessionStart is lifecycle-aware (#175). Harnesses emit SessionStart several times per logical conversation, so `_handle_session_start` branches on the hook input's `source`: `startup`/`clear` (or absent) serve the full snapshot; `resume`/`compact` serve a **delta** — only notes the logical-session chain (#180) hasn't been served yet, one line each, plus a single "N notes already in context" line. The already-served set is read from the chain's raw retrieval streams (each member folder's `retrieval_log.jsonl` + live buffers) — the same files `context_served` projects from, current the moment an event is buffered. Each startup event records the `lifecycle` source and a `disposition` (`served_full` / `served_delta` / `skipped_replay`); delta events also stamp `chain_root` (the chain's earliest segment's note id), and the projection keys those rows to the root — a resume/compact delivery's exposure belongs to the logical conversation, not the segment that buffered it. Replay guard: a delivery's identity is `(session_id, lifecycle, payload-bytes hash)`, stamped as `delivery_id` on the served event; a byte-identical redelivery (double registration, post-archival replay) injects nothing and records a `skipped_replay` event with zero ids, so exposure is never double-counted. The receipt survives live-buffer archival because it rides inside the event line into `retrieval_log.jsonl`. Prompt-time pushed context (`prompttime`) and agent-requested retrieval (`onthefly`) are different surfaces and are untouched by all of this.
