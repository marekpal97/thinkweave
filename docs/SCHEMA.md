...
| `context_served` | Notes served to a session, projected from `retrieval_log.jsonl`. `source` ∈ `startup` / `onthefly` / `prompttime` / `loop-prime` (issue-loop claim-time priming, #57) / `codex-startup` (#107) — each push source stays distinct from agent-pulled `onthefly` to preserve the agent-judgment signal for the RLVR export. `/learn`'s and `/brief`'s own MCP reads land as ordinary `onthefly` rows; neither surface writes a dedicated source (dec-696bacfb). A startup event carrying `chain_root` (#175 — a resume/compact delta delivery) keys its rows to the chain's root session id, not the segment that buffered it. | `(session_id, note_id, source)` PK, `ts` | `indexer.py:126` |
