...
| `context_served` | Notes served to a session, projected from `retrieval_log.jsonl`. `source` ∈ `startup` / `onthefly` / `prompttime` / `loop-prime` (issue-loop claim-time priming, #57) / `codex-startup` (#107) / `brief` / `learn` (the `/brief` and `/learn` serving surfaces, #120 — `tool: brief|learn` in the retrieval log) — each push source stays distinct from agent-pulled `onthefly` to preserve the agent-judgment signal for the RLVR export. | `(session_id, note_id, source)` PK, `ts` | `indexer.py:126` |
