...
| `context_served` | Notes served to a session, projected from `retrieval_log.jsonl`. `source` ∈ `startup` / `onthefly` / `prompttime` preserves the agent-judgment signal for the RLVR export. | `(session_id, note_id, source)` PK, `ts` | `indexer.py:126` |
