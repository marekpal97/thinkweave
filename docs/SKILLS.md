...
|---|---|---|---|---|
| `/wrap` | session_extraction | — | — | Inline-compose insights/decisions → `weave_extract` → `weave wrap-finalize` deterministic tail (prune → index → judge → landing → drift). Live mode interactively; catch-up mode is invoked nightly by `dream-wrap-worker` (no separate cron). |
| `/brief` | daily_orientation | — | — | Daily orientation — a live meta layer over the nightly digests, pull-only. One Bash `weave health --json` (jobs/queues with per-lane feeder binding/hooks/digest freshness), then the existing retrieval tools — `weave_search` (watermark + landings), `weave_timeline`, `weave_prompts` — and judgment narration (dec-696bacfb: no collect payload, no render_plan; quiet day = five lines; stale digest = banner first, brief still produced from raw landings). One `weave_create(type=digest, kind: brief)` = the next watermark; no mark step — a brief surveys what is new, so logging its citations as served context would double-count demand. Focus precedence = asked (`weave_prompts` probes) ▸ done (window concept edges) ▸ declared (`RESEARCH_FOCUS.md` gaps + `PRIORITIES.yaml`) — declared only surfaces as "you said you wanted X — here's what landed". Never heals unasked. |
| `/dream` | vault_synthesis | — | — | Two-phase subagent orchestrator. **Phase 1** (synthesis) fans out 5 workers in parallel — `dream-{promotion,merge,theme,essence,priority}-worker` — merges plan fragments, runs `weave dream apply` (one index rebuild, one maintenance.jsonl line). **Phase 2** (composition + consumption) fans out 6 workers in dependency waves — `dream-wrap-worker` (catch-up unwrapped sessions) + `dream-judge-worker` (drain rejudge queue) + `dream-seam-link-worker` (stitch cross-parent linkage on freshly-folded hubs via `weave hubs apply-linkage`) + `dream-seam-worker` (reconcile the CC-auto-memory↔vault **memory seam** — judge each dirty CC fact's vault twin, write the durable map via `weave seam commit`) + `dream-outcome-worker` (judge issue-loop trajectory PR outcomes via `weave trajectory judge`) in parallel, then `dream-digest-worker` (compose vault-global `type: digest` notes at `vault/digests/YYYY-MM-DD-<grain>.md`, one per non-empty grain). One cron entry replaces three (`/dream`, `/wrap` catch-up, `/judge-prediction --drain`). Headless-safe; never prompts. |
...
| `/research` | url_routing | paper, repo, article, news, youtube, podcast | import | Atomic-unit front door for one URL — classifies via `url_patterns` and dispatches to `research-paper/-repo/-article/-youtube/-podcast` (or `/news` for news outlets). Not a rail; the same atomic units run inside `/drain`. |
...
| `/learn` | vault_tutoring | — | — | Vault-grounded tutor — one retrieval partitioned into the user's trajectory vs the world's material; test-first on revisit; closes with one learn note (the session's own MCP retrieval calls land in `context_served` through standard logging). Interactive. See [Lifecycles §Learn note](LIFECYCLES.md#learn-note-kind-learn). |
...
| Worker | What it does |
|---|---|
| `dream-wrap-worker` | Catch up unwrapped sessions (subsumes the standalone `/wrap` cron). |
...
| `dream-outcome-worker` | Judge issue-loop trajectory notes for PR outcomes via `weave trajectory judge` — phase 1 (`merged-clean`/`reworked`/`closed-unmerged`/`routed-to-human` at merge/close) and phase 2 (rework-blame + revert at +`dream.trajectory_phase2_days`). Appends `prediction_history`-shaped entries + an `outcome_label`; the reward signal for the self-improvement loop. |
...
> Path A research subskills (`research-paper`, `research-repo`, `research-article`, `research-youtube`, `research-podcast`) are sequential skills, not `Task` subagents — they live under `commands/research/` and are invoked by `/research` or `/drain` directly. `research-youtube` / `research-podcast` resolve a one-shot URL (oEmbed / RSS feed), enqueue the resolved item, and drain one `research-{youtube,podcast}-worker` — the one-shot analog of the `/youtube` and `/podcast` orchestrators.
