...
| `/learn` | vault_tutoring | — | — | Vault-grounded tutor — one retrieval partitioned into the user's trajectory vs the world's material; test-first on revisit; closes with one learn note + `context_served(source='learn')`. Interactive. See [Lifecycles §Learn note](LIFECYCLES.md#learn-note-kind-learn). |
