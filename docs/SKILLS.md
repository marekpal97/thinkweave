...
| `/learn` | vault_tutoring | — | — | `/learn <topic>` — vault-grounded tutor (#171). One retrieval (`weave learn coverage`) partitioned by provenance into the user's **trajectory** (the spine; zero → explicit first-contact line, never a fabricated recap) and the world's **material**; revisit (prior `kind: learn` note) opens test-first, new topic teach-first; thin material → capped in-session fill (`[learn] fill_cap`, default 3) via the real research workers, overflow enqueued. Twelve-rule teaching contract lives in the skill verbatim. Closes with one learn note (`weave_create` + `weave learn check`), `context_served(source='learn')` via `weave learn mark`, unanswered questions → probes via `weave learn probe`. Interactive; not headless. See [Lifecycles §Learn note](LIFECYCLES.md#learn-note-kind-learn). |
