...
| `/brief` | daily_orientation | — | — | Daily orientation — a live meta layer over the nightly digests, pull-only. One Bash `weave brief collect --json` (watermark → health → landings per lane → three-layer focus vector → catalysts / theme deltas / essence rewrites → attention → connections, plus the `render_plan`), one narration that renders exactly the non-empty sections (CONTRADICTIONS & EXTENSIONS lead; quiet day = five lines; stale digest = banner first, brief still produced from raw landings), one `weave_create(type=digest, kind: brief)` = the next watermark, one `weave brief mark` logging every surfaced id as `context_served(source='brief')`. Focus = asked (probes) ▸ done (window concept edges) ▸ declared (`RESEARCH_FOCUS.md` gaps + `PRIORITIES.yaml`) — declared only surfaces as "you said you wanted X — here's what landed". Never heals unasked. |
