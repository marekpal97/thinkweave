...
from personal_mem.core.config import Config, user_cache_dir
from personal_mem.scheduling.registry import ScheduledJob, resolve_command

FENCE_START = "# --- personal-mem cron block ---"
FENCE_END = "# --- end personal-mem ---"
...
class CrontabBackend:
    ...
    def uninstall(self, jobs: list[ScheduledJob] | None = None) -> None:
        ...
        :class:`~personal_mem.scheduling.taskscheduler.TaskSchedulerBackend`
...
def _splice(existing: str, block: str) -> str:
    """Replace the fenced personal-mem block in ``existing`` with ``block``.
