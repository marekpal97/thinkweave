"""Concept operations — unified action dispatcher for the `mem_concepts` MCP tool.

Folds five previously separate MCP tools into one action-dispatched call:
- ``action='list'`` (default — was mem_concepts)
- ``action='tighten'`` (was mem_concepts_tighten)
- ``action='merge'`` (was mem_concepts_merge)
- ``action='search'`` (was mem_concept_search — now delegates to graph.walk)
- ``action='source_counts'`` (was mem_concept_source_counts)
- ``action='drift'`` (was mem_concepts_drift)
"""
