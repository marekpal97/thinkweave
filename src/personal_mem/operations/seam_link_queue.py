...
``/dream`` apply or ``mem concepts merge``; theme merge via
...
the revisions through ``mem hubs apply-linkage`` (which validates via
``validate_linkage_revision``), and clears the stamp.

Same shape and guarantees as ``operations/rejudge_queue.py`` — one
per-vault JSONL at ``vault/.mem/seam_link_queue.jsonl``, idempotent
...
_QUEUE_PATH = ".mem/seam_link_queue.jsonl"
...
def dequeue(config: Config, *, hub_kind: str, hub_id: str) -> bool:
    ...
    Called by ``mem hubs apply-linkage --clear-fold`` — clearing the
