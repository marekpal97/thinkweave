...
``vault/.mem/queues/<source_type>.jsonl``, the rejudge queue is *one*
queue per vault, rooted at ``vault/.mem/rejudge_queue.jsonl``. It feeds
...
def drain_all(config: Config) -> list[dict[str, Any]]:
    ...
    Used by ``mem judge --drain``. After this returns, the queue file is
...
def pending_due(
    ...
    *,
    ...
    than ``age_days`` from now (UTC). Used by ``mem judge --drain`` to merge
