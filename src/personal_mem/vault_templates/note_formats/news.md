<!--
  Note-format template for source_type: news
  Brief skeleton the research-news-worker follows (~400-700 words, dense,
  evidence-rich). Seeded into your vault at vault/config/note_formats/news.md on
  `mem init` — edit that copy directly to reshape every news brief (user-owned,
  survives upgrades). Sections guide the LLM writer; rename, add, drop, or reorder
  freely. The two `## Vault Connections` variants are picked by the triage verdict.
-->
