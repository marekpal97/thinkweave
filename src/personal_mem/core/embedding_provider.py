...
The completion wrapper (:mod:`personal_mem.core.agent_client`) covers
...
``mem index --embed --reset`` clears the cache.
...
Provider matrix (initial set):
  ...
  • ``sentence_transformer`` — ``all-MiniLM-L6-v2`` (384-dim, ~80 MB,
    local). Opt-in via ``pip install personal-mem[embeddings-local]``.
  • ``litellm`` — pass-through to any LiteLLM-supported embedding
    provider (Voyage, Cohere, etc.). Opt-in via
    ``pip install personal-mem[embeddings-litellm]``.
"""
    ...
    def embed(self, texts: list[str]) -> list[list[float]]:
        ...
        try:
            ...
            raise ImportError(
                ...
                "pip install personal-mem[embeddings]"
            ) from exc
