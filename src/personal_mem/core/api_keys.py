...
Lookup order (first non-empty wins):
  ...
  2. ``.env`` at ``$PERSONAL_MEM_VAULT/.env`` (vault-scoped secrets)
  3. ``.env`` at ``Path.cwd()/.env`` (project-local development)
  4. ``.env`` at the personal_mem project root (back-compat with
...
# Personal_mem project root — the back-compat fallback for developers
...
def get_provider_key(provider: str) -> str | None:
    ...
    locations (vault → cwd → personal_mem project root) only if the key
...
def _load_env_files() -> None:
    ...
    # Back-compat with ``enrich.load_openai_api_key()`` — the personal_mem
