"""``mem judge`` — drain the rejudge queue, list pending decisions, manual rejudge.

The Python side of the ``/judge-prediction`` skill. Three flavours:

- ``mem judge --drain --json`` — pull the supersession-triggered queue,
...
- ``mem judge --rejudge <dec-id>`` — enqueue one decision manually, then
  shell to ``claude -p "/judge-prediction --decision <dec-id>"``.
- ``mem judge --list-pending`` — read-only enumeration of decisions whose
...
def cmd_judge(args: argparse.Namespace) -> None:
    """Dispatch the ``mem judge`` subcommand on which flag was set."""
    cfg = load_config()
    ...
    print(
        "usage: mem judge {--drain | --rejudge DEC | --list-pending} [--max N] [--json]",
        file=sys.stderr,
    )
    sys.exit(2)
