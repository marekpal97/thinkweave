"""``mem doctor --mcp`` — diagnose personal-mem MCP registration.
...
def check_registration_scopes(cwd: Path) -> CheckResult:
    """Report which scopes declare personal-mem; FAIL if >1 conflict."""
    ...
    if not scopes:
        return CheckResult(
            ...
            passed=False,
            detail="personal-mem is not registered in any scope",
            fix="run `mem install --yes` (machine) or install the plugin",
        )
    ...
    if len(scopes) == 1:
        ...
        return CheckResult(
            name="registration scopes",
            passed=True,
            detail=f"1 scope ({scope_name}) declares personal-mem",
        )
    ...
    if len(keys) > 1:
        return CheckResult(
            ...
            passed=False,
            detail=(
                f"{len(scopes)} scopes declare personal-mem with DIFFERENT "
                f"invocations: {summary} — Claude Code will pick one and warn"
            ),
            fix=(
                "reconcile to a single shape (re-run `mem install --yes` and "
    ...
    return CheckResult(
        name="registration scopes",
        passed=True,
        detail=f"{len(scopes)} scopes declare personal-mem identically ({summary})",
    )
...
def check_launcher_resolves(cwd: Path, timeout_s: float = 5.0) -> CheckResult:
    ...
    if machine_entry is not None:
        ...
        return CheckResult(
            ...
            fix="register personal-mem first (see scope check above)",
        )
    ...
    if shutil.which(cmd) is None and not Path(cmd).is_absolute():
        return CheckResult(
            ...
            fix=(
                ...
                "or re-run `mem install --yes` to pin an absolute path"
            ),
        )
    ...
    try:
        ...
        return CheckResult(
            ...
            fix="install uv or re-run `mem install --yes`",
        )
...
def check_vault_env() -> CheckResult:
    """``PERSONAL_MEM_VAULT`` (if set) must point at an existing dir."""
    ...
    if not Path(raw).expanduser().is_dir():
        return CheckResult(
            ...
            fix="`mkdir -p $PERSONAL_MEM_VAULT && mem init`",
        )
    return CheckResult(
        name="PERSONAL_MEM_VAULT",
        passed=True,
...
def check_mem_mcp_on_path() -> CheckResult:
    """Informational: ``mem-mcp`` console script on PATH. Not fatal —
    the canonical invocation is ``uv run … mem-mcp`` which doesn't need
    it. Reported as a hint when the launcher probe fails on PATH.
    """
    found = shutil.which("mem-mcp")
    if found:
        return CheckResult(
            name="mem-mcp on PATH",
            passed=True,
    ...
    return CheckResult(
        ...
        detail=(
            "not on PATH (informational only — `uv run … mem-mcp` is the "
