"""``mem seam`` — memory-seam maintenance CLI (the worker's Bash hands).

Two actions over :mod:`personal_mem.synthesis.memory_seam`:

- ``mem seam surface [--json]`` — re-emit the cheap, embedding-free dirty
  ...
  scan JSON.
- ``mem seam commit --verdicts <path>|- [--json]`` — the write path. Takes
  ...
  and writes ``vault/.mem/memory_seam.json`` + the rendered
  ``vault/.mem/memory_seam.md``. Facts the worker didn't rule on keep their
  prior verdict; removed facts drop out.

The expensive half — twin resolution + the actual confirmed/stale/diverged
judgment — lives in the worker turn (``mem_search(mode='similar')`` + LLM
...
def cmd_seam(args: argparse.Namespace) -> None:
    ...
    if action == "surface":
        ...
        print("Usage: mem seam {surface|commit}", file=sys.stderr)
        sys.exit(2)
