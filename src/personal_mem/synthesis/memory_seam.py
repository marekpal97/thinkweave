...
2. diffs them against the durable state map (``vault/.mem/memory_seam.json``)
...
``dream-seam-worker`` resolves twins via ``mem_search(mode='similar')`` in its
own turn and writes its verdicts back through ``mem seam commit``. The split
...
STATE_RELPATH = Path(".mem") / "memory_seam.json"
REPORT_RELPATH = Path(".mem") / "memory_seam.md"
...
# (e.g. /home/x/python_projects/personal_mem →
#  -home-x-python_projects-personal-mem). For DISPLAY we strip the home
...
def _build_query(fm: dict, body: str) -> str:
    """The text the worker hands to ``mem_search(mode='similar')``.
...
def collect_cc_facts() -> list[dict]:
    ...
    ``query``, ``mem_type``). ``key`` is ``<dir_slug>::<slug>`` — slugs can
    ...
    if CC_PROJECTS_ROOT.is_dir():
        for d in sorted(CC_PROJECTS_ROOT.iterdir()):
            mem = d / "memory"
            if mem.is_dir():
                dirs.append((d.name, mem))

    for dir_slug, mem_dir in dirs:
...
def stale_prior(fact: dict, *, stale_age_days: int, now: datetime) -> bool:
    ...
    """
    if str(fact.get("mem_type")) != "project":
        return False
...
# ---------------------------------------------------------------------------
# State rebuild (the `mem seam commit` core) + renderer
# ---------------------------------------------------------------------------
