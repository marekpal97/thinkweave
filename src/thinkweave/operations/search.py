...
CLI search handler and the `weave_search` MCP tool delegate here.
...
def _project_buffer_session_ids(cfg: Config, project: str) -> set[str]:
    ...
    Used to scope active ``.weave/buffer/<uuid>.jsonl`` files so we never
...
def query_prompts(
    ...
    2. Active ``.weave/buffer/<session_uuid>.jsonl`` files whose owning
    ...
    buffer_root = cfg.weave_dir / "buffer"
