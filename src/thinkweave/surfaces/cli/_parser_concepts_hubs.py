...
def add_concepts_subparsers(sub) -> None:
    ...
    p_prune_singletons = concepts_sub.add_parser(
        ...
        help=(
            ...
            "step in /weave-resolve-concepts."
        ),
    )
    ...
    p_proposed = concepts_sub.add_parser(
        ...
        help=(
            ...
            "Use with /weave-resolve-concepts to find promotion candidates."
        ),
    )
    ...
    p_drift.add_argument(
        ...
        help=(
            ...
            "judgment lives in /weave-resolve-concepts)."
        ),
    )
    p_drift.add_argument(
        "--hub-jaccard",
...
def add_hubs_subparsers(sub) -> None:
    ...
    p_hubs_plan.add_argument(
        "--out", default="", help="Plan output path (default: .weave/hubs_plan.json)"
    )
...
def add_drain_subparsers(sub) -> None:
    p_queue = sub.add_parser(
        "queue",
        help="Inspect per-source-type acquisition queues (.weave/queues/*.jsonl)",
    )
    ...
    p_drain = sub.add_parser(
        "drain",
        help=(
            "Drain a queue or backfill concept hubs. Replaces `weave hubs run` "
