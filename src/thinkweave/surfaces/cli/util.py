"""``weave init`` / ``prune-orphans`` / ``sources`` — small administrative commands."""
...
def cmd_mcp(args: argparse.Namespace) -> None:
    ...
    Thin shim around ``thinkweave.surfaces.mcp.server.main``. Used by
    the Claude Code plugin shell (``plugin.json`` invokes ``weave mcp``).
    """
    from thinkweave.surfaces.mcp.server import main as mcp_main
...
def _seed_vault_templates(vault_root: Path) -> None:
    ...
    """
    pkg_root = Path(__file__).resolve().parents[1].parent  # → .../src/thinkweave
...
def cmd_prune_orphans(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        print(f"Warning: index cleanup failed — run `weave index --full` to rebuild. ({e})")
...
def cmd_sources(args: argparse.Namespace) -> None:
    from thinkweave.acquisition.sources import REGISTRY, all_specs, get_spec, load_user_specs
    from thinkweave.surfaces.cli.skill import skills_for_source_type
    ...
    if action == "list":
        ...
        print()
        print(
            "To add a new source type: run `weave sources scaffold <slug> "
...
def _cmd_sources_scaffold(
    ...
    if slug in registry:
        print(
            ...
            f"(in src/thinkweave/acquisition/sources/registry.py). Pick a different slug.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if user_yaml.exists():
        ...
        header = (
            "# thinkweave user-side source-type registry\n"
            "# Each top-level key is a source_type slug. Edit by hand to\n"
            "# change bucket/layout/aliases, or run `weave sources scaffold` again.\n"
            "\n"
        )
    ...
    if skill_target == "user":
        ...
        repo_root = pkg_root.parent.parent  # .../thinkweave
    ...
    # clobbered on the next `weave`/plugin upgrade. Seed the vault file from the
    ...
    print(
        ...
        f"`weave sources show {slug}` to confirm."
    )
