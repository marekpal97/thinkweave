"""``weave schedule`` — install thinkweave's recurring jobs onto the host's
...
registry and dispatch through :func:`thinkweave.scheduling.select_backend`,
...
from thinkweave.scheduling import load_jobs, scheduling_path, select_backend
from thinkweave.scheduling.cron import CrontabBackend
from thinkweave.scheduling.taskscheduler import TaskSchedulerBackend
...
def cmd_schedule(args: argparse.Namespace) -> None:
    ...
    if not all_jobs:
        print(
            ...
            f"{scheduling_path(cfg)} (run `weave init` to seed it)."
        )
        return
...
def _cmd_list(backend, jobs: list) -> None:
    ...
    print("Preview the rendered scheduler entries with "
          "`weave schedule install --dry-run`.")
...
def _cmd_uninstall(backend, jobs: list, *, dry_run: bool) -> None:
    if dry_run:
        if isinstance(backend, CrontabBackend):
            print("# Would remove the thinkweave crontab fence block.")
        else:
            for job in jobs:
    ...
    print(f"Removed thinkweave schedule via {backend.name}.")
