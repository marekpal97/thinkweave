"""``weave hubs link`` — temporal-DAG linkage pass.
...
:func:`thinkweave.core.agent_client.batch_completions_sync`. The
``weave hubs link --via inline`` route dispatches to the
...
def hubs_link(cfg, args: argparse.Namespace) -> None:
    ...
    if reindex_failures:
        print(
            ...
            f"Run `uv run weave index` to catch up."
        )
...
def hubs_apply_linkage(cfg, args) -> None:
    """``weave hubs apply-linkage`` — validated linkage writes for one hub.
    ...
    try:
        ...
        try:
            ...
            idx.close()
    except Exception:  # noqa: BLE001 — contention: `weave index` catches up
        reindexed = False
    ...
    if getattr(args, "json", False):
        ...
        print(
            ...
            + ("" if reindexed else " · reindex deferred (run `weave index`)")
        )
