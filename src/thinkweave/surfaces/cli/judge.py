...
def _rejudge_argv(dec_id: str, ns: str | None) -> list[str]:
    ...
    route is active. ``ns`` is the harness profile's ``namespace()`` — pass
    ``None`` for the project-scope clone, where bare names resolve.
    """
    prompt = namespace_prompt(f"/judge-prediction --decision {dec_id}", ns)
    return active_harness().headless_argv(prompt)
...
def _cmd_rejudge(cfg, args: argparse.Namespace) -> None:
    ...
    result = subprocess.run(
        _rejudge_argv(dec_id, active_harness().namespace()),
        stdin=sys.stdin,
        stdout=sys.stdout,
        stderr=sys.stderr,
    )
