"""MCP server for thinkweave.

Run: ``python -m thinkweave.surfaces.mcp.server``
Transport: stdio
Requires: ``pip install thinkweave[mcp]``
...
(``weave_concept_search``, ``weave_source_lens``, ``weave_decisions_for_file``,
``weave_concepts_tighten``, ``weave_concepts_merge``,
``weave_concept_source_counts``, ``weave_concepts_drift``) were deleted
...
from thinkweave.surfaces.mcp.tools.extract import (
...
def main() -> None:
    try:
        ...
        print("MCP server requires: pip install thinkweave[mcp]", file=sys.stderr)
        sys.exit(1)

    import asyncio

    from thinkweave.core.config import load_config
    from thinkweave.surfaces.mcp.tools import all_schemas, dispatch

    server = Server("thinkweave")
    cfg = load_config()
...
def build_server():
    ...
    try:
        ...
        raise RuntimeError("MCP server requires: pip install thinkweave[mcp]") from exc
