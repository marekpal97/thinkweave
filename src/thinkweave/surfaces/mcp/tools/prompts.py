"""``weave_prompts`` MCP tool.

Read-only listing of user prompts captured by the UserPromptSubmit hook
(Phase 4 E). Backed by :func:`thinkweave.operations.search.query_prompts`,
...
def handle(cfg: Config, args: dict) -> str:
    """Run ``weave_prompts`` and return a JSON-serialisable payload as a string.
