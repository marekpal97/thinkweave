...
The hygiene rails (dream drift v2, `/weave-resolve-concepts`,
...
- a **concept centroid** is the mean of the cached embeddings of the
  notes carrying that concept (``note_concepts`` ⨝ ``.weave/embeddings.db``);
...
- **cosine pairs** above a threshold are surfaced to the model with an
  ...
  merge worker judges from contents without extra ``weave_read`` round-trips.

Judgment memory lives in the existing dream maintenance log
(``vault/.weave/maintenance.jsonl``) — :func:`judged_pairs` reads the
...
def _load_note_vectors(cfg: Config) -> dict[str, list[float]]:
    """note_id → embedding from the ``.weave/embeddings.db`` cache.

    Returns ``{}`` when the cache is missing — callers degrade to
    string-only detection rather than failing the scan.
    """
    from thinkweave.core.embeddings import _unpack_embedding
...
def judged_pairs(cfg: Config) -> set[PairKey]:
    ...
    ``vault/.weave/maintenance.jsonl``. Excluding these from the scan is
...
def judged_clusters(cfg: Config) -> tuple[set[tuple[str, str]], set[ClusterKey]]:
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        for c in verdicts.get("coarsenings") or []:
            ...
            for weave in c.get("members") or []:
                ml = str(weave).lower().strip()
                if ml and ml != target:
