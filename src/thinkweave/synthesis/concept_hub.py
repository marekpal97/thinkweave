...
- ``weave hubs plan`` / ``weave hubs run`` — bulk backfill via the OpenAI SDK
...
def render_concept_hub(
    ...
    *,
    ...
    if domains:
        from thinkweave.synthesis.concepts import _domain_label
...
# ---------------------------------------------------------------------------
# Migration helpers — exposed at this surface for `weave index --full`.
# ---------------------------------------------------------------------------
...
def migrate_concept_hub_headings(config: Config) -> int:
    ...
    rewritten on this invocation. Wired into ``weave index --full`` so the
...
# ---------------------------------------------------------------------------
# Planner — used by `weave hubs plan`
# ---------------------------------------------------------------------------
...
def build_plan(
    ...
    *,
    ...
    """
    from thinkweave.synthesis.concepts import concept_to_domains, load_ontology
