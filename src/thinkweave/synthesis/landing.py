...
``thinkweave.acquisition.sources.config.DEFAULT_CONFIG`` under the
...
def _default_landing_filenames() -> dict[str, str]:
    ...
    :data:`thinkweave.acquisition.sources.config.DEFAULT_CONFIG` so the framework
    has a single source of truth for landing-doc names."""
    from thinkweave.acquisition.sources.config import DEFAULT_CONFIG
...
def _get_db(config: Config):
    ...
    if not db_path.exists():
        raise FileNotFoundError(
            f"Index not found at {db_path}. Run `weave index` first."
        )
...
def _gather_prompt_probes(
    ...
    # Active (non-archived) buffers in .weave/buffer. We can't trivially map a
...
def state_of_play_context(config: Config, project: str) -> str:
    ...
    from thinkweave.operations.dream import recent_dream_reports
...
def generate_all(config: Config, project: str) -> dict[str, str]:
    ...
    Filenames respect ``vault/.weave/sources.yaml: landing_files:`` so a
...
def write_landing_docs(
    ...
    Args:
        ...
        landing_filenames_override: Optional explicit mapping that
            ...
            ``vault/.weave/sources.yaml`` via :func:`landing_filenames`.
