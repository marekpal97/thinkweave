...
def save_aliases(config: Config, aliases: dict[str, list[str]]) -> Path:
    ...
    lines = ["# Concept aliases: canonical → [aliases]",
             "# Auto-maintained by `weave concepts tighten/merge`", ""]
...
def _seed_ontology_path() -> Path:
    ...
    ``src/thinkweave/vault_templates/config/ontology.yaml`` (moved there in
...
def _vault_ontology_path() -> Path | None:
    ...
    A file still at ``<vault_root>/.weave/ontology.yaml`` raises
    :class:`LegacyConfigLocationError` (caught here and returns None).
    """
    try:
        from thinkweave.core.config import load_config, resolve_config_file
...
def load_tag_vocabulary(path: Path | None = None) -> set[str]:
    ...
    fall outside this set are surfaced as drift by ``weave doctor``.
    """
    raw = _parse_ontology_file(path)
...
def load_domain_markers() -> frozenset[str]:
    ...
    union the vault override at ``<vault>/.weave/ontology.yaml::domain_markers``.
    Vault list extends — never replaces — the built-ins so thinkweave
...
def split_concepts_by_ontology(
    ...
    *,
    ...
    Used at every concept-write surface (weave_extract, enrich, importers)
    so non-ontology vocabulary can never reach canonical ``concepts:``
    without explicit promotion via ``/weave-resolve-concepts``.
    """
...
def prune_noisy_singletons(
    ...
    *,
    ...
    ``/weave-resolve-concepts``, never via automated count-based pruning.
...
def promote_proposed_concept(
    ...
    *,
    ...
    1. Add the term to ``vault/.weave/ontology.yaml`` under ``domain``
...
def generate_concept_hub_skeletons(
    ...
    """
    from thinkweave.synthesis.concept_hub import ensure_concept_hub_skeleton
...
def generate_hub_pages(
    ...
    paths (keyed by concept). Used by ``weave concepts hubs``.
    """
...
def add_hub_wikilinks(
    ...
    from thinkweave.synthesis.landing import landing_filename_set
...
def delete_concept_hub(config: Config, concept: str) -> bool:
    ...
    Used after ``weave concepts merge`` so the renamed concept's hub
...
def fold_concept_hub_on_merge(
    ...
    *,
    ...
    by both merge paths (``weave concepts merge`` and the ``/dream`` apply
    step). Three moves:

    1. **Fold** — the loser hub's catalyst log + essence are merged into
       the winner hub (:func:`thinkweave.synthesis.hub.fold_hub_logs`:
...
def hub_archive_dir(config: Config) -> Path:
    ...
    logic required.
    """
    from thinkweave.synthesis.concept_hub import topics_dir
...
def find_redundant_hub_candidates(
    ...
    *,
    ...
    """
    from thinkweave.synthesis.concept_hub import parse_concept_hub, topics_dir
...
def format_drift_report(report: dict) -> str:
    ...
    if dupes:
        ...
        for a, b, reason in dupes:
            lines.append(
                f"  drift: '{a}' ≈ '{b}' ({reason}) — suggest: `weave concepts merge {a} {b}`"
            )
    ...
    if report.get("ontology_stale"):
        lines.append(
            ...
            "run `weave concepts hubs` to refresh hub pages"
        )
...
# ---------------------------------------------------------------------------
# Doctor — broader vault coherence linter (`weave doctor`)
# ---------------------------------------------------------------------------
...
def find_isolated_notes(db) -> dict:
    ...
    Isolated notes can't be reached via ``weave_graph`` walks and don't
...
def format_doctor_report(report: dict) -> str:
    ...
    if stale:
        ...
        lines.append("  weave index --embed --only-new")
        lines.append("")
    ...
    if isolated is not None:
        ...
        if total == 0:
            ...
            lines.append(
                ...
                "unreachable via weave_graph and absent from concept-shared "
                "retrieval):"
            )
            ...
            lines.append(
                "  Likely fix: `weave enrich` on the 0-1 concept bucket "
