"""Theme registry — ``vault/.weave/themes.yaml``.

Structural symmetry with the concept ontology (``vault/.weave/ontology.yaml``):
...
def rebuild(config: Config) -> int:
    ...
    write ``vault/.weave/themes.yaml``. Returns the count of themes written.
...
def load(config: Config) -> dict[str, dict[str, Any]]:
    """Read ``vault/.weave/themes.yaml`` and return a dict keyed by thm-id.
