...
The index is a derived artifact — always rebuildable via `weave index --full`.
...
-- consumed by weave_concepts(action='canonical_for', concept=X).
...
-- <date>* entry points at (written by `weave hubs link`); ``cited_note_id``
-- is the entry's citation. Derived from markdown (the shared Hub parser);
-- rebuildable via `weave index --full` — markdown stays truth. ``seq`` is
...
-- Live consumers (recent_probe_details, weave_prompts) intentionally stay
...
-- events.jsonl via ``weave index --full`` — the JSONL stays truth.
...
class Indexer:
    ...
    def db(self) -> sqlite3.Connection:
        if self._db is None:
            self.config.weave_dir.mkdir(parents=True, exist_ok=True)
            ...
            # concurrent writers (e.g. `weave hubs run` applying entries while
    ...
    def _init_schema(self) -> None:
        ...
        # untouched, so a fresh `weave index --full` after upgrade is enough
    ...
    def rebuild(self, full: bool = False) -> dict:
        ...
        ``rebuild(full=True)`` / ``weave index --full`` heals it periodically.
        """
        ...
        # ``session.md`` is rewritten by ``weave_extract`` each wrap, so the
    ...
    def _rebuild_edges_incremental(self, changed_ids: set[str]) -> int:
        ...
        **Staleness trade-off:** if an OLD unchanged note links (frontmatter
        ...
        ``weave index --full``) recomputes everything and heals this on its
    ...
    def _rebuild_context_served(self, only_ids: set[str] | None = None) -> None:
        ...
        Args:
            only_ids: When supplied, restrict the projection to session notes
                ...
                lands in ``changed_ids`` naturally (``weave_extract`` rewrites
    ...
    def _rebuild_prompts(self, only_ids: set[str] | None = None) -> None:
        ...
        Covers *archived* buffers only; live ``.weave/buffer`` files are
    ...
    def _project_session_prompts(
        ...
        *probe* rows only — attribution for instructions would be noise
        ...
        file is missing.
        """
        from thinkweave.core.events import extract_prompts, match_probe_concepts
    ...
    def _rebuild_co_served_edges(self, threshold: int = 2) -> int:
        ...
        ``weave_graph(edge_types=[..., 'co_served'])``; default walks
        unchanged.
    ...
    def materialize_links(self, max_links: int = 7, dry_run: bool = False) -> dict:
        ...
        """
        from thinkweave.core.vault import strip_section
