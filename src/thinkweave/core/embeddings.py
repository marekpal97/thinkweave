...
SQLite database (.weave/embeddings.db). Never called during hooks.

Requires: pip install thinkweave[embeddings]  (httpx)
"""
...
class EmbeddingSearch:
    ...
    def clear(self) -> int:
        ...
        Backs ``weave index --embed --reset`` — the escape hatch for a
    ...
    def latest_embedded_at(self) -> str | None:
        ...
        ``compute_all(only_new=True)`` and ``weave doctor`` to drive the
    ...
    def _call_api(self, texts: list[str]) -> list[list[float]]:
        ...
        via :func:`thinkweave.core.embedding_provider.build_from_vault`.
        """
        if not texts:
            return []
    ...
    def _provider(self):
        ...
        if cached is not None:
            return cached
        from thinkweave.core.embedding_provider import build_from_vault
