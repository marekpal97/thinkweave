...
``vault/.weave/sources.yaml`` lets a user adjust the per-source-type behaviour
of thinkweave (queue paths, importer/skill bindings, dedup keys, drain
...
``vault_templates/.weave/sources.yaml`` — top-level scalars, top-level
...
raises ``ValueError`` so the user finds out at ``weave doctor``/``weave init``
...
def load_user_config(vault_root: Path | None) -> dict[str, Any]:
    ...
    A file still sitting at ``<vault_root>/.weave/sources.yaml`` raises
