...
Every source type in thinkweave is described by a single ``SourceTypeSpec``
entry here. ``VaultManager.create_note`` reads the registry to decide how to
route a new source note on disk; CLI commands (``weave sources list/show``)
...
dropping entries into ``<vault_root>/.weave/source_types.yaml`` — see
``load_user_specs`` below and the ``weave sources scaffold`` CLI command.
...
class SourceTypeSpec:
    ...
    Attributes:
        ...
        skills: filenames (without ``.md``) under ``commands/`` that handle
            this source type. Informational only — used by ``weave sources
            show`` to cross-reference.
        description: one-liner shown by ``weave sources list``.
...
REGISTRY: dict[str, SourceTypeSpec] = {
    ...
    "conversation": SourceTypeSpec(
        ...
        layout="flat",
        description=(
            "ChatGPT conversation exports. Imported via `weave import chatgpt`."
        ),
        temporal_grain="none",
    ),
...
def normalize(source_type: str, vault_root: Path | None = None) -> str:
    ...
    ``<vault_root>/.weave/source_types.yaml`` are consulted alongside the
...
def get_spec(
    ...
    User-side specs (from ``<vault_root>/.weave/source_types.yaml``) are
...
def load_user_specs(vault_root: Path) -> dict[str, SourceTypeSpec]:
    """Read ``<vault_root>/.weave/source_types.yaml`` and parse SourceTypeSpec
    entries.
    ...
    ``weave doctor`` is where the real surfacing happens).
    """
