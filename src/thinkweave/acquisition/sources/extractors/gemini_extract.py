...
shell out via Bash rather than importing Python:
    ...
    python -m thinkweave.acquisition.sources.extractors.gemini_extract youtube <url>
    python -m thinkweave.acquisition.sources.extractors.gemini_extract podcast <audio_url>
...
Requires ``GOOGLE_API_KEY`` and the ``[gemini]`` optional dep group::
    ...
    pip install thinkweave[gemini]
    export GOOGLE_API_KEY=...

The SDK import is lazy so the rest of thinkweave stays usable when
    ...
    *,
    ...
    if not key:
        return {
            ...
            "reason": "GOOGLE_API_KEY env var not set (checked env + .env in THINKWEAVE_VAULT and CWD)",
        }
    ...
    try:
        ...
        return {
            ...
            "reason": (
                ...
                "run `pip install thinkweave[gemini]`"
            ),
        }
...
def _download_audio(url: str) -> Path:
    ...
    req = Request(url, headers={"User-Agent": "thinkweave/0.1 podcast-fetch"})
...
def _maybe_load_env_file() -> None:
    ...
    Looks for ``.env`` at, in order:
      1. ``$THINKWEAVE_VAULT/.env`` — vault-scoped secrets
