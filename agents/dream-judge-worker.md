...
The list is capped at the configured `dream.rejudge_cap` (default 20) (read from `.weave/rejudge_queue.jsonl` plus stale `pending` verdicts found by the scan; phase 1's `weave dream apply` removes the handed-off file entries from the queue, so they won't resurface — entries beyond the cap survive for the next cycle). Process every entry in input order; do not sub-select.
...
- `reason == "supersession"` — the entry was enqueued because a successor decision declared `supersedes: [<this dec-id>]`. The successor is the most-load-bearing evidence; `weave_read(<predecessor_decision_id>)` should be a no-op (it's this decision itself) so instead search the decision's frontmatter `supersedes_by` field or `weave_read` the successor explicitly if its id is known.
...
### Step E — Write back via `weave_update`
...
Repeat A–E for every entry in `rejudge_queue`. One `weave_update` per entry, even for `unevaluable` — the history line records that the judge looked and couldn't decide, which prevents the same item resurfacing every drain.
...
- `outcome.errors` — per-entry errors that prevented a verdict (decision missing, weave_update raise, etc.). Use sparingly; most exceptions belong here.
- Top-level `errors` — worker-level errors not tied to a specific entry. Use sparingly.
- `side_effects` — one `decision_updated` per successful weave_update. The orchestrator's report consumes this; do not omit.
...
- **Decision id missing in vault** (`weave_read` returns no content) → record `{"decision_id": "...", "reason": "decision not found"}` under `outcome.errors`. Don't `weave_update` — the id doesn't resolve.
- **`predicted_outcome` empty / missing** → emit `unevaluable` with `reason: "no predicted_outcome to evaluate"`. Still `weave_update` (so the entry doesn't resurface).
- **Bash/Grep-required pointer** → emit `unevaluable` with `reason: "manifestation pointer requires Bash/Grep evidence (worker scope)"`. Still `weave_update`. The `/judge-prediction` standalone skill can pick this up next cycle with its full tool surface.
- **`weave_update` raises** → record `{"decision_id": "...", "reason": "<exception text>"}` under `outcome.errors`. The rejudge_queue.jsonl line is consumed by phase 1's `weave dream apply` (the hand-off point), not by you — if weave_update fails the verdict is simply lost for this cycle and the decision's `prediction_match` stays at its prior state; a still-`pending` decision self-heals via the next scan's stale-pending index sweep, which re-surfaces it.
- **Frontmatter prediction_history corrupted (not a list)** → coerce to `[]` before appending your entry. The weave_update will overwrite the field with a clean list; record `{"decision_id": "...", "reason": "prediction_history coerced from <type>"}` as a benign note (still in `outcome.errors` for visibility).
