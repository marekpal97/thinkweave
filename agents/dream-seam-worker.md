...
description: Phase-2 of /dream — reconcile Claude Code auto-memory against the vault; resolve each dirty CC fact's twin, judge confirmed-fresh/stale/diverged/durable-unique, and write the durable map via `weave seam commit`; emits one outcome JSON line.
tools: Read, Write, Bash, mcp__thinkweave__weave_search, mcp__thinkweave__weave_read
model: sonnet
color: green
---
...
```
{
  ...
  "memory_seam": {
    "dirty": [
      {
        ...
        "slug": "<fact slug>",
        "weave_type": "feedback|project|user|reference|...",
        "description": "<one-line fact summary>",
        "query": "<text to hand weave_search>",
    ...
    "report_path": "/abs/vault/.weave/memory_seam.md",
    "state_path":  "/abs/vault/.weave/memory_seam.json"
  }
}
```

You judge only `dirty`. `removed` facts need no judgment — `weave seam commit` recomputes the map from the *current* CC files, so a removed fact simply isn't there. The commit also carries forward every clean (non-dirty) fact's prior verdict untouched; you never see or re-rule those.
...
1. **Resolve the twin.** Call `weave_search(query=<entry.query>, mode="similar", limit=5)`. The top result's `rank` is the cosine. (Whole-vault — twins are NOT co-located by the fact's project; do not pass a project filter.)
...
2. **Band the cosine** against `thresholds`:
   ...
   - in-between → ambiguous; `weave_read` the top candidate and decide whether it's the same fact or a mere neighbour.

3. **Inspect the twin** (`weave_read` the top candidate when cosine warrants). Compare the CC fact's *claim* against the twin's *current* state — its `status`, dates, counts, and body. Assign one verdict:
...
**`weave seam commit` is the ONLY write path.** Do not Write to `memory_seam.json`/`.md` by hand — the content hashes and carry-forward logic are the commit's job, and a hand-written map will desync the next cycle's dirty diff. Do not modify the CC memory files themselves; you only read them (indirectly, via the surface).
...
```json
{"worker": "dream-seam-worker", "cycle_id": "dream-YYYYMMDD-HHMMSS-xxxxxx", "phase": 2, "outcome": {"judged": 7, "verdicts": {"confirmed-fresh": 4, "stale": 1, "diverged": 1, "durable-unique": 1}, "removed": 0, "report_path": "/abs/.../memory_seam.md"}, "side_effects": [{"kind": "file_written", "path": ".weave/memory_seam.json"}, {"kind": "file_written", "path": ".weave/memory_seam.md"}], "errors": []}
```

Conventions:
- `outcome.judged` — count of dirty facts you ruled on.
- `outcome.verdicts` — your per-verdict tally for the facts you judged this cycle (not the whole-map totals; `weave seam commit`'s JSON carries those if you want them).
- `outcome.removed` — `len(memory_seam.removed)` (informational; the commit handled them).
- `side_effects` — the two files `weave seam commit` wrote (relative vault paths).
- `errors` — empty on success; on a `weave seam commit` failure put the exception text here and leave the verdict tally as what you intended to write.
...
- **`weave_search` returns nothing** (cold/empty index, or a genuinely novel fact) → `durable-unique`. Don't crash, don't retry the search more than once.
...
- **`weave seam commit` raises** → emit `error`, put the exception text under `errors`, leave the map as-was (the prior cycle's map is still valid).

## What this worker does NOT do

- Do NOT edit the CC auto-memory files or the vault notes. You are read-only on both sides; the only write is the durable map via `weave seam commit`.
...
- Do NOT touch the dream maintenance log or report — those are `weave dream apply`'s.
