# Thinkweave — Architecture

The narrative reference hub. CLAUDE.md is the thin in-session operating guide; the lookup catalogs (lifecycles, skills, CLI/MCP) live under `docs/`. This doc carries the conceptual story — how the layers fit, why the abstractions are shaped the way they are.
...
- [Capability lanes](#capability-lanes)
  ...
  - [Dream orchestrator (two-phase, mirrors /drain)](#dream-orchestrator-two-phase-mirrors-drain)
  - [Memory seam — CC auto-memory ↔ vault reconciliation](#memory-seam--cc-auto-memory--vault-reconciliation)
- [Ontology as the joint vocabulary](#ontology-as-the-joint-vocabulary)
- [Adding a new source type](#adding-a-new-source-type)
- [Themes vs concept hubs](#themes-vs-concept-hubs)
- [Queue primitive](#queue-primitive)
- [User configuration layout — `vault/config/`](#user-configuration-layout--vaultconfig)
- [Discovery strategies](#discovery-strategies)
- [Decision lifecycle](#decision-lifecycle)
- [Coherence — how the vault avoids duplication](#coherence--how-the-vault-avoids-duplication)
- [Invocation surface](#invocation-surface)
- [Operations layer](#operations-layer)
- [LLM provider abstraction](#llm-provider-abstraction--coreagent_clientpy--coreembedding_providerpy)
- [A note on the importers](#a-note-on-the-importers-under-srcthinkweaveacquisitionimporters)
...
| `CLAUDE.md` | Agents in-session | Thin operating guide: what Thinkweave is, the retrieval cheatsheet, operational rules, and a reference index pointing here + to `docs/`. |
| `ARCHITECTURE.md` (this) | Contributors | Layer boundaries, source primitive, capability lanes, coherence mechanics — the conceptual narrative. |
...
Thinkweave splits cleanly into two layers with a one-way dependency.
...
└────────────────────────────────────┬────────────────────────────────────┘
                                     ...
                                     ▼
┌─────────────────────────────────────────────────────────────────────────┐
│ Knowledge layer (src/thinkweave/)                                     │
...
Each of the six packages declares its interface in its `__init__.py`: a curated set of re-exports in `__all__` (the demand-measured workhorses, e.g. `from thinkweave.core import Config, VaultManager`) plus a front-door docstring in four parts — **operations** (what you call), **invariants** (what must hold), **storage** (what it owns on disk), **extension points** (where new members plug in). Code *outside* a package imports its workhorses from the door, not from submodules; code *inside* a `thinkweave` package keeps submodule paths — a door import from a sibling package can hit the door mid-initialization (e.g. `core`'s door is still binding names while `core.indexer` reaches into synthesis), so `from thinkweave.core import Indexer` is only safe from outside. Anything not on the door is reachable by submodule path but is not interface. The `core`, `operations`, and `surfaces` doors resolve their heavy names lazily (PEP 562 `__getattr__`) to protect the hook hot path: `weave-hook` fires as a fresh process on every prompt/tool-use/stop, its handler bodies import operations modules on each fire, and an eager door would drag the CLI, all operations siblings, and their retrieval/synthesis/acquisition imports into that budgeted path (pinned by the weight tests in `tests/test_package_doors.py`). The synthesis door is provisional (marked in-file) until Track A finalizes it — its names are stable even while implementations move. Two omissions (`acquisition`'s `importers`/`discover`, `surfaces`' `mcp`) are deliberate — see those door docstrings for the cycle and import-weight reasons. The contract is pinned by `tests/test_package_doors.py`.
...
A **source** is a note of `type: source` representing external content — a paper, repo, article, newsletter post, conversation export. Every source type is declared once in `src/thinkweave/acquisition/sources/registry.py` as a `SourceTypeSpec`:
...
Skills declare their lane via YAML frontmatter `capabilities: [...]`. `weave skill list` reads these headers.
...
flowchart LR
    ...
    D[/discover/]:::st -->|gap or enqueue| Q[queue JSONL<br/>vault/.weave/queues/]:::store
    Q -->|peek + claim| DR[/drain/]:::st
    DR -->|Path A: sequential Skill<br/>Path B: Task subagent fan-out| W[Worker]:::st
    W -->|weave_create| N[source note]:::store
...
Some flows legitimately skip discover: `/substack` and `weave import {chatgpt|claude-history}` because the user (or an external export) has already done the discovery step; `/news <url>` and `/research <url>` because they're one-shot URL bypasses.
...
flowchart LR
    classDef st fill:#1f2937,stroke:#60a5fa,color:#f9fafb,rx:6,ry:6
    classDef store fill:#0f172a,stroke:#a78bfa,color:#f9fafb,rx:6,ry:6
    SC[weave dream scan]:::st --> P1[Phase 1<br/>5 synthesis workers]:::st
    P1 -->|plan fragments| AP[weave dream apply]:::st
    AP --> P2A[Phase 2 wave A<br/>wrap + judge + seams]:::st
    P2A --> P2B[Phase 2 wave B<br/>digest]:::st
    AP --> ML[(maintenance.jsonl)]:::store
    P2B --> DG[(digests/YYYY-MM-DD-{concept,event}.md)]:::store
```
...
**Operational vs epistemic separation (per dec-719e47e0 + n-d31cc330).** The dream report at `vault/reports/dream/<cycle_id>.md` is *operational* (what apply did this cycle). The digests at `vault/digests/YYYY-MM-DD-{concept,event}.md` are *epistemic* (what your knowledge gained today). The epistemic layer is two-tier: a **narrative lead** (`## In brief` synthesizes the through-line; `## Most actionable` ranks the day's arrivals against *behavioral* focus — projects with sessions in the last 14d + concepts under probe pressure, recomputed each cycle rather than read from a hand-maintained list) over an **inventory detail** (the grouped, wikilinked receipts). Same orchestrator, separate workers, separate output files, separate prompt framings — no data-level conflation despite shared dispatch.
...
- **`synthesis/memory_seam.py`** (embedding-free) walks the CC memory files into fact records keyed by content hash + mtime, diffs them against the durable state map (`vault/.weave/memory_seam.json`) to find the **dirty** set (new / edited / previously-unresolved / recheck-due), computes the deterministic `project`-type+age stale prior, and renders the report. It resolves no twins and calls no embedding API — so the dream *scan* keeps its API-free contract.
- **`dream-seam-worker`** (phase 2, Wave A) does the irreducibly-semantic half: for each dirty fact it resolves the vault twin via `weave_search(mode='similar')`, bands the cosine (`seam.cosine_twin` / `seam.cosine_none`), reads the twin, and rules `confirmed-fresh` / `stale` / `diverged` / `durable-unique`. It writes verdicts back through **`weave seam commit`**, which recomputes the durable map from the *current* CC files (so a verdict can never attach to stale text), carries forward clean facts' priors, and renders `vault/.weave/memory_seam.md`.
...
Loading is centralised in `src/thinkweave/synthesis/concepts.py:load_ontology` — a minimal YAML parser with no external dependencies. Skills read the file directly (no MCP round-trip) because it's small and changes rarely.

New terms a skill encounters go into `proposed_concepts`, not `concepts`. The gate is **server-enforced** — `weave_extract`, `weave_create`, and the importers all run incoming concept lists through the merged ontology and shunt non-matches to `proposed_concepts` automatically. Promotion to canonical requires `/tighten` review (or the nightly `dream-promotion-worker`; threshold: config `dream.promotion_threshold`, default `count ≥ 5`).
...
The five-step pattern — registry entry (`SourceTypeSpec` in `sources/registry.py`) → config defaults (`sources.yaml`) → skill file (`commands/<slug>.md`) → optional research subskill / worker → smoke test — is all it takes; the `podcast-events` / `podcast-concepts` pair is the worked reference (registry spec + config block, zero skill code, because `/podcast` discovers every `podcast-*` type from config). Nothing else in the framework should need to change.
...
| Resolution skill | `/tighten` (both families) | `/tighten` (both families) |
| Storage | `vault/concepts/topics/{name}.md` | `vault/themes/{thm-X}-{slug}.md` |

The disambiguation test, registry mechanics (`themes.yaml`), and per-source-type theme floating (event-grain vs concept-grain) live in [docs/LIFECYCLES.md §"Theme"](docs/LIFECYCLES.md#theme) — that's the agent-facing rules-of-the-road. ARCHITECTURE.md only carries the structural hub-spine view.
...
All human-edit configuration files live under `vault/config/`. The hidden `vault/.weave/` directory is for runtime/derived state only (SQLite DBs, JSONL queues, batch buffers, logs).
...
Loaders use a backwards-compatible fallback (`vault/config/<filename>` → `vault/.weave/<filename>`) so pre-Phase-3.1 vaults keep working. Writes always commit forward to `vault/config/`. Legacy-location reads emit a one-per-session stderr deprecation warning.
...
trajectory_phase2_days = 14        # closed-horizon delay before the loop-trajectory delayed-signal (rework-blame) pass
trajectory_rework_threshold = 0.5  # merged-diff rewrite fraction above which phase-2 rules reworked-post-merge
trajectory_agent_identities = "claude, noreply@anthropic.com"  # substrings marking a PR commit as agent-produced
...
`vault/config/sources.yaml` overlays per-vault defaults onto the shipped `DEFAULT_CONFIG` in `src/thinkweave/acquisition/sources/config.py`. Four top-level sections, all optional:
...
The optional `vault/config/source_types.yaml` overlay (loaded by `sources/registry.py`) registers new `SourceTypeSpec` entries at runtime — vault-side source-type extensions without forking the framework. This mirrors the open/closed asymmetry above: the overlay opens the *registry* (a new type appears), while drain/dedup behaviour stays closed-world until the type is fully specified.

`weave_sources_config` MCP exposes the merged dict to skills that don't want to re-parse the YAML themselves. The CLI exposes `weave sources list` / `weave sources show <slug>` for the registry view.
...
Each strategy lives in its own file under `src/thinkweave/acquisition/discover/strategies/` and implements the `Strategy` protocol (`_protocol.py`) — adding a new one is **one file plus one `register()` line** in `strategies/__init__.py`. This directory is the framework's growth axis post-launch.
...
```mermaid
stateDiagram-v2
    [*] --> proposed: weave_create / weave_extract<br/>(outcome: abandoned/partial,<br/>or outcome: committed with<br/>no matching session commit)
    proposed --> accepted: weave_extract finds<br/>commit matching file_paths
    accepted --> superseded: new decision declares<br/>supersedes: [dec-X]
    proposed --> superseded: same as above
    accepted --> deprecated: weave_update<br/>status=deprecated
    proposed --> deprecated: same as above
```

- `synthesis/judge.py` is **read-only** — emits a verdict (`kept`/`superseded`/`reverted`/`unknown`) from structural evidence (was the file committed? did tests pass? was it re-edited later?). Never writes back. Verdict-to-status writeback lives in `operations/decisions.weave_judge_and_writeback` (`kept→accepted`, `superseded→superseded`, `reverted→deprecated`, `unknown→no change`).
- The only auto-flip in the system is the `supersedes`-declared one above, where the writer made the relationship explicit.
- Decisions can carry a `predicted_outcome:` prose string with claim + manifestation pointer. The `/judge-prediction` skill (not an API call — the running session IS the judge) writes verdicts to `prediction_history`.

The full 4-state transition table, evidence-gated supersession doctrine (2026-06-13), and the predicted-outcome / RLVR substrate (five verdicts, three invocation paths, the context-served projection, and the `weave rlvr export` feed) live in [docs/LIFECYCLES.md §"Decision"](docs/LIFECYCLES.md#decision).
...
|---|---|---|
| Concept overlap (near-dupes) | drift v2: string rules ∪ centroid-cosine ≥ 0.8 (`synthesis/geometry.py`), verdict-history-excluded | `/dream` scan (`drift_pairs`); `weave doctor` / `weave concepts drift` for the advisory view |
| Concept merge | rename across notes + FOLD hub into winner + archive tombstone + seam-link enqueue | `weave concepts merge`, `/dream` apply (`merges`) |
...
| Theme dedup | drift v2 cosine over theme embeddings → `merge_theme_into` (fold + repoint + `merged-into:` tombstone) | `/dream` (`theme_dup_candidates` → `theme_merges`); `/tighten` on demand |
