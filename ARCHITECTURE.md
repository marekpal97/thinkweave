...
Each of the six packages declares its interface in its `__init__.py`: a curated set of re-exports in `__all__` (the demand-measured workhorses, e.g. `from thinkweave.core import Config, VaultManager`) plus a front-door docstring in four parts — **operations** (what you call), **invariants** (what must hold), **storage** (what it owns on disk), **extension points** (where new members plug in). Code *outside* a package imports its workhorses from the door, not from submodules; code *inside* a `thinkweave` package keeps submodule paths — a door import from a sibling package can hit the door mid-initialization (e.g. `core`'s door is still binding names while `core.indexer` reaches into synthesis), so `from thinkweave.core import Indexer` is only safe from outside. Anything not on the door is reachable by submodule path but is not interface. The `core` and `surfaces` doors resolve their heavy names lazily (PEP 562 `__getattr__`) so the hook hot path — `weave-hook` fires on every prompt/tool-use/stop — never pays for the CLI or the indexer's upward edge into synthesis. The synthesis door is provisional (marked in-file) until Track A finalizes it — its names are stable even while implementations move. Two omissions are deliberate and documented in the door docstrings: `acquisition` does not re-export `importers`/`discover` (their top-level `core.vault` imports would close a cycle through `vault.py`'s registry import), and `surfaces` does not re-export `mcp` (a single-consumer console script; door placement would add only import weight). The contract is pinned by `tests/test_package_doors.py`.
