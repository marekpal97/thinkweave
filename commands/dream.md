...
Returns a `DreamCycleScan` JSON payload with all phase-1 scan surfaces (`promotion_candidates`, `drift_pairs`, `theme_cluster_signals`, `theme_dup_candidates`, `theme_log_gaps`, `essence_candidates`, `recent_probes`, plus phase-2 surfaces `unwrapped_sessions`, `rejudge_queue`, `seam_link_queue`, `trajectory_outcomes`, `knowledge_delta`).
...
- **Wave A** — every task whose `depends_on` is empty (or all its named workers are not in the enabled list). For the current registry this is `dream-wrap-worker`, `dream-judge-worker`, `dream-seam-link-worker` (stitches cross-parent catalyst linkage on hubs folded by this or an earlier cycle's merges; drains its own queue via `weave hubs apply-linkage --clear-fold`), `dream-seam-worker` (reconciles the CC-auto-memory↔vault **memory seam** — resolves each dirty CC fact's vault twin via `weave_search(mode='similar')`, rules confirmed-fresh / stale / diverged / durable-unique, and writes the durable map + report via `weave seam commit`), and `dream-outcome-worker` (judges issue-loop trajectory notes for their PR outcome — deterministically, via `weave trajectory judge`: phase 1 at merge/close, phase 2's rework-blame + revert at +`dream.trajectory_phase2_days`; appends `prediction_history`-shaped entries + an `outcome_label` that `weave rlvr export` consumes alongside decisions). The memory-seam surface is computed embedding-free in the scan; its `dirty` list is the per-fact judgment payload. The `trajectory_outcomes` surface is the has-signal trigger; the worker's rail re-scans internally.
