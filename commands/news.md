...
- This skill does **not** run the post-batch `theme_scan` hook. Single-item ingest doesn't need a cluster check; theme floating runs naturally on the next `/drain` cycle. To force a scan, run `mem themes scan-candidates --source-type news` directly.
