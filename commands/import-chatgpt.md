...
The user invokes this from `weave import chatgpt --via inline <path>` (the
...
2. **Walk conversations.** For each conversation:
   - Flatten the `mapping` tree into a chronological transcript
     ...
     `python -c "from thinkweave.acquisition.importers.chatgpt import parse_thread; …"`.
   ...
   - Call `mcp__thinkweave__weave_create(note_type='source', title=<conversation title>, body=<summary markdown>, concepts=<list>, extra_frontmatter={"source_type": "chatgpt", "chatgpt_id": <id>, "created": <iso date>})`.
...
- Dedup: the importer's batch path keys on `(conversation_id, title)`. Before
  `weave_create`, call `mcp__thinkweave__weave_search(query=<title>, type='source')` to
  catch the already-imported case.
- Don't pull message bodies into `weave_create`'s `body` — only the structured
...
- The `concepts` you assign go through the strict ontology gate. Use
  `mcp__thinkweave__weave_concepts(action='list', limit=200)` once at the
