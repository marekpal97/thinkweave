...
```
--verdicts '[{"prompt": "<the prompt'\''s opening words, verbatim>", "register": "correction"}, ...]'
```
...
### C5. Prompt verdicts — precision over recall
Downstream consumers trust these labels (RLVR export for feedback; probe pressure and `/discover` for probes), so a false non-neutral is worse than a miss:
