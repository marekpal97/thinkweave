...
| `landings` / `lanes` | notes that landed, grouped by `source_type`; per-lane verdict `kept` · `ran_nothing_kept` · `dead` · `unknown` |
...
| `no_news` | Only lanes with state `dead`: *"<lane>: nothing since <since> and its job `<job>` is stale/missing — the cron likely died."* Fold `ran_nothing_kept` lanes into one sentence ("paper/article ran and kept nothing"). `unknown` lanes get nothing. |
