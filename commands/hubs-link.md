---
description: Inline temporal-DAG linkage for concept hubs — walk hubs lacking agrees/contradicts/extends flags and rewrite via the running model. The `mem hubs link --via inline` path; pairs with `--via batch` which fans out via the API wrapper.
allowed-tools: Read, Bash, mcp__personal-mem__mem_read, mcp__personal-mem__mem_update
---
...
Walks the same hub set `mem hubs link --via batch` processes, but
produces revisions via the running session's model.

The user invokes this from `mem hubs link --via inline` (the CLI prints
...
1. **Identify candidate hubs.** Run `Bash`:
   ```bash
   mem hubs link --dry-run [--concept <slug>] [--min-entries 2]
   ```
...
2. **Walk hubs one at a time.** For each candidate concept:
   ...
   - Rewrite the entries in place with the new flags. Use `Bash` to call
     ...
     ```bash
     python -c "
     from personal_mem.synthesis.concept_hub import parse_concept_hub, write_concept_hub
     ...
     "
     ```
     Or call `mcp__personal-mem__mem_update` only if the hub note carries a
     standard `id:` (some hubs do — fall back to the Python one-liner when
     they don't).

3. **Reindex.** After all hubs are updated, run
   `mem index --only-new` so the new edges land in the SQLite index.
